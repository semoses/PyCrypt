...
def drawString(canvas, data):
    canvas.create_text(data.width//2, data.spacing, text = "Encrypt/Decrypt ROT13 File", 
...
def timerFired(data):
     if data.writeNextLine == 2 and data.over == False:
        try:
            encodeTextFileROT13(data.inputFilename, data.outputFilename)
            data.over = True
        except:
...
# From 112 Website
################################################################################
def runEncryptROT13File(width=300, height=300):
