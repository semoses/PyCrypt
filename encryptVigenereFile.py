...
def drawString(canvas, data):
    canvas.create_text(data.width//2, data.spacing, text = "Encrypt Vigenere File", 
...
def timerFired(data):
    if data.writeNextLine == 3 and data.over == False:
        try:
            encodeTextFileVigenere(data.inputFilename, data.outputFilename, data.keyword)
            data.over = True
        except:
...
# From 112 Website
################################################################################
def runEncryptVigenereFile(width=300, height=300):
...
# runEncryptVigenereFile(500, 500)
