...
def drawString(canvas, data):
    canvas.create_text(data.width//2, data.spacing, text = "Decrypt Route File", 
...
def timerFired(data):
    if data.writeNextLine == 3 and data.over == False:
        try:
            decodeTextFileRoute(data.inputFilename, data.outputFilename, data.numOfCols)
            data.over = True
        except:
...
# From 112 Website
################################################################################
def runDecryptRouteFile(width=300, height=300):
