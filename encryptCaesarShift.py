...
def init(data):
    ...
    data.decodedMessageCopy = data.decodedMessage
    data.encodedMessage = encodeCaesarShift(data.decodedMessage, data.shift)
...
def keyPressed(event, data):
    if data.startAnimation == 0:
        ...
        data.decodedMessage = data.message
        data.decodedMessageCopy = data.decodedMessage
        data.encodedMessage = encodeCaesarShift(data.message, data.shift)
    ...
    if data.startAnimation == 2:
        ...
        if data.count % data.speed == 0:
            ...
            if data.encodedMessage[i] != data.decodedMessage[i]:
                data.decodedMessage = incrementLetter(data.decodedMessage, i)
                data.currShift += 1
            elif data.index < (len(data.decodedMessage) - 1):
                data.index += 1
                data.currShift = 0
            else:
...
def runEncryptCaesarShift(width=300, height=300):
