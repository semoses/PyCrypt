...
def drawString(canvas, data):
    canvas.create_text(data.width//2, data.spacing, text = "Encrypt Keyword File", 
...
def timerFired(data):
    if data.writeNextLine == 3 and data.over == False:
        try:
            encodeTextFileKeyword(data.inputFilename, data.outputFilename, data.keyword)
            data.over = True
        except:
...
def keyPressed(event, data):
    if data.writeNextLine == 0:
        ...
        if (event.char in string.ascii_letters or event.char == " ") and \
        event.char.upper() not in data.keyword.upper():
...
# From 112 Website
################################################################################
def runEncryptKeywordFile(width=300, height=300):
