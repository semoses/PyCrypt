...
def drawString(canvas, data):
    ...
    index = 0
    canvas.create_text(data.width//2, data.spacing, text = "Encrypt Keyword", 
...
def updateAnswerString(data):
    ...
    while len(data.answerString) < len(data.encodedMessage):
        data.answerString += " "
...
def runEncryptKeyword(width=300, height=300):
