...
def drawString(canvas, data):
    canvas.create_text(data.width//2, data.spacing, text = "Encrypt/Decrypt Atbash File", 
...
def timerFired(data):
     if data.writeNextLine == 2 and data.over == False:
        try:
            encodeTextFileAtbash(data.inputFilename, data.outputFilename)
            data.over = True
        except:
...
# From 112 Website
################################################################################
def runEncryptAtbashFile(width=300, height=300):
