...
def drawString(canvas, data):
    canvas.create_text(data.width//2, data.spacing, text = "Decrypt Vigenere Cipher", 
...
def highlightGridRowColBox(canvas, data):
    ...
    # block
    canvas.create_rectangle(data.margin, y0 + space, data.boxWidth 
...
def init(data):
    ...
    data.decodedMessage = decodeVigenere(data.encodedMessage, data.keyword)
...
def keyPressed(event, data):
    if data.startAnimation == 0:
        ...
        data.encodedMessage = data.message
        data.decodedMessage = decodeVigenere(data.message, data.keyword)
            ...
            if data.count % data.speed == 0:
                ...
                if data.messageCopy[data.index] in string.ascii_lowercase:
                    if data.highlightCol != string.ascii_lowercase.index(data.keywordCopy[data.index]):
                        data.highlightCol += 1
                    elif (data.highlightRow + data.highlightCol)%data.lenOfAlphabet \
...
def runDecryptVigenere(width=300, height=300):
