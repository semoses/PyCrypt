...
def drawString(canvas, data):
    canvas.create_text(data.width//2, data.spacing, text = "Decrypt Keyword File", 
...
def timerFired(data):
    if data.writeNextLine == 3 and data.over == False:
        try:
            decodeTextFileKeyword(data.inputFilename, data.outputFilename, data.keyword)
            data.over = True
        except:
...
# From 112 Website
################################################################################
def runDecryptKeywordFile(width=300, height=300):
