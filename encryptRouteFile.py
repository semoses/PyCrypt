...
def drawString(canvas, data):
    canvas.create_text(data.width//2, data.spacing, text = "Encrypt Route File", 
    ...
    canvas.create_text(data.spacing, data.spacing*7, 
        text = "Columns: %d" % data.numOfCols, fill = data.fill,
        font = "Courier 20", anchor = W)
...
def timerFired(data):
    if data.writeNextLine == 3 and data.over == False:
        try:
            encodeTextFileRoute(data.inputFilename, data.outputFilename, data.numOfCols)
            data.over = True
        except:
...
def keyPressed(event, data):
    if data.writeNextLine == 0:
        ...
        if event.char in string.digits:
            data.numOfCols = (data.numOfCols*10) + int(event.char)
        elif event.keysym == "BackSpace":
            data.numOfCols //= 10
        elif event.keysym == "Return":
            data.writeNextLine += 1
...
# From 112 Website
################################################################################
def runEncryptRouteFile(width=300, height=300):
