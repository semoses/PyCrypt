...
def drawString(canvas, data):
    canvas.create_text(data.width//2, data.spacing, text = "Decrypt Caesar File", 
...
def timerFired(data):
    if data.writeNextLine == 3 and data.over == False:
        try:
            decodeTextFileCaesar(data.inputFilename, data.outputFilename, data.shift)
            data.over = True
        except:
...
# From 112 Website
################################################################################
def runDecryptCaesarFile(width=300, height=300):
