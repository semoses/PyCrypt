...
def drawString(canvas, data):
    canvas.create_text(data.width//2, data.spacing, text = "Encrypt Route Cipher", 
    ...
    canvas.create_text(data.width//2, data.height//2 + data.spacing*5,
        text = "Columns: %d" % (data.numOfCols), font = "Courier 30",
...
def init(data):
    ...
    data.boxWidth = 30
    data.numOfRows = (len(data.message) // data.numOfCols) + 1
...
def keyPressed(event, data):
    if data.startAnimation == 1:
        ...
        if event.keysym == "BackSpace":
            ...
            if data.charCount > 0:
                data.charCount -= 1
        data.message = data.message[0:data.charCount]
        if len(data.message) >= data.numOfCols:
            data.numOfRows = (len(data.message) - 1) // data.numOfCols + 1
        if data.message != "":
        	data.encodedMessage = encodeRoute(data.message, data.numOfCols)
        if event.keysym == "Return":
            data.startAnimation += 1
    elif data.startAnimation == 0:
        if event.keysym in "Up" and data.numOfCols < 10:
            data.numOfCols += 1
        elif event.keysym == "Down" and data.numOfCols > 2:
            data.numOfCols -= 1
        if len(data.message) >= data.numOfCols:
            data.numOfRows = (len(data.message) - 1) // data.numOfCols + 1
        data.margin = (data.width - (data.boxWidth * data.numOfCols)) / 2
	...
	if data.startAnimation == 2 and data.animationOver == False:
		if data.count % data.speed == 0:
			...
			if data.row == (data.numOfRows - 1) and data.col == (data.numOfCols - 1):
				...
				if data.row == (data.numOfRows - 1):
					data.col += 1
					data.row = 0
					index = data.col + (data.row * data.numOfCols)
					if index < len(data.message):
						data.answerString += data.message[index]
				else:
					data.row += 1
					index = data.col + (data.row * data.numOfCols)
					if index < len(data.message):
...
def runEncryptRoute(width=300, height=300):
